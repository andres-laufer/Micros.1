...
PSECT udata_shr ;memoria
    w_temp: DS 1
    status_temp: DS 1


;-------------------------------Instruciones vect reset ------------------------
...
config_interrup:
    ...
    bsf ((INTCON) and 07Fh), 3 ;interrupción en puerto b
    bcf ((INTCON) and 07Fh), 0
    bsf ((INTCON) and 07Fh), 5 ;interrupción en Timer0
    bcf ((INTCON) and 07Fh), 2
    return
...
int_OCB: ;Configuración de botones
    ...
    decf PORTA
    bcf ((INTCON) and 07Fh), 0 ;Limpieza de los botones
    return
...
contador_centenas:
    ...
    subwf var, F
    incf Cont_Cent
    btfsc ((STATUS) and 07Fh), 0 ;Se repite el proceso si no hay overflow
    goto $-3
    decf Cont_Cent
    addwf var
    movf Cont_Cent,w
